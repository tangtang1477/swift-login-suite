...
type Step = "initial" | "email-login" | "email-signup";
...
const LoginPage = () => {
  ...
  const handleContinue = (e: FormEvent) => {
    ...
    setEmailError(eErr);
    if (eErr) return;
    // Simulate checking if user exists — go to login step
    setStep("email-login");
  };
        ...
        {step === "initial" && (
          <>
            ...
            <form onSubmit={handleContinue} noValidate>
              ...
              <button
                type="submit"
                className="mt-4 w-full flex items-center justify-center gap-2 transition-all duration-150 ease-out hover:brightness-110 active:translate-y-px"
                style={{
                  height: "40px",
                  background: "#71F0F6",
